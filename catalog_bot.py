...
# --- Конфигурация ---
API_ID = 30394715
API_HASH = "81ee020c7e55609b24131f6e702237dd"
...
async def send_movie(callback: types.CallbackQuery):
    ...
    if msg_id:
        # пересылаем сообщение и получаем объект Message
        copied = await bot.copy_message(chat_id=callback.message.chat.id,
                                        from_chat_id=MOVIES_CHAT_ID,
                                        message_id=msg_id)
        # если это видео — отправляем как send_video
        if copied.video:
            await bot.send_video(chat_id=callback.message.chat.id,
                                 video=copied.video.file_id,
                                 caption=movie.get("title", ""))
